...
libname d 'E:\Laura\Work\COGA\Projects\INT_scale\Paper\data\cssagaiv';
...
* keeping the CSSAGAIV items needed to score the INT scale;
data tmp2; set master.&dataset;
	...
	PT1a5-PT1a6 PT1a7K PT1a7Ka PT1a7Kb PT1a8-PT1a15 PT1a16K1 PT1a16K2 PT1a17-PT1a19 PT3-PT7 PT9K PT9K1 PT10-PT15 PT11a PT17-PT20 PT20Ka PT21 PTp23f PTp23h PTp23i PTp23j 
	/*Changes from SSAGAIV: PT1a-PT1a4 and PT1a20 are omitted, PT1a7 becomes PT1a7K, PT1a7Ka, and PT1a7Kb, PT1a16 becomes PT1a16K1 and PT1a16K2PT9, 
	PT9 is replaced by PT9K and PT9K1 and PT20Ka is added*/
	DP1 DP2 DP2a DP4a DP4b DP4c DP5 DP6Kb1 DP6KB2 DP7 DP7f DP8 DP9Ka DP9Kb DP10 DP11k DP11ka DP12 DP13k DP13ka DP15ka DP15kb DP15c DP15d DP27 DP27b1-DP27b3 DP27b5-DP27b13 
	/*Changes from SSAGAIV: DP6b is replaced by DP6KB1 and DP6KB2, DP9 by DP9Ka and DP9Kb, DP11 by DP11k and DP11Ka, DP13 by DP13k and DP13ka, DP15a by DP15ka, and DP15b by DP15kb*/
...
* generating INT Scale Score for CSSAGA-IV;
data d.v21_int_&dataset; 
	...
	* 0-3 score;
	...
	else if OCcpD4dx=5 & OCobD4dx=5 then INT4=.;
	...
	****************************************;
	...
	PT1a_cnt=0;
	PT1a_0cnt=0;
	array PT1a (18) PT1a5-PT1a6 PT1a7K PT1a7Ka PT1a7Kb PT1a8-PT1a15 PT1a16K1 PT1a16K2 PT1a17-PT1a19;
	do i=1 to 18;
	...
	else if PT3=. & PT4=. & PT5=. & PT6=. & PT7=. & PT1a_0cnt=18 then INT5_1=.;
	else INT5_1=0;
	
	sumINT5_2=sum(PT9K=5 | PT9K1=5, PT10=5, PT11=5 & PT11a^=5, PT12=5, PT13=5, PT14=5, PT15=5);
	if sumINT5_2>2 & PT_ko1^=1 then INT5_2=1;
	else if PT9K=. & PT9K1=. & PT10=. & PT11=. & PT11a=. & PT12=. & PT13=. & PT14=. & PT15=. & PT1a_0cnt=18 then INT5_2=.;
	else INT5_2=0;

	sumINT5_3=sum(PT17=5, PT18=5, PT19=5, PT20=5 | PT20Ka=5, PT21=5);
	if sumINT5_3>1 & PT_ko1^=1 then INT5_3=1;
	else if PT17=. & PT18=. & PT19=. & PT20=. & PT20Ka=. & PT21=. & PT1a_0cnt=18 then INT5_3=.;
	else INT5_3=0;
	...
	*********************************;
	...
	/* start of code taken from DP_DSMIVrv.txt for CSSAGAIV */
	...
	/* end of code taken from DP_DSMIVrv.txt for CSSAGAIV */
	...
	if DP1=. & DP2=. & (DP2a=. | DP2a=.K) then
		...
		do;
			...
			if DP6Kb1=5 | DP6Kb2=5 then INT6_2=1; else INT6_2=0;
			if DP7=5 | DP7f=5 then INT6_3=1; else INT6_3=0;
			if DP8=5 | DP9Ka=5 | DP9Kb=5 then INT6_4=1; else INT6_4=0;
			if DP10=5 then INT6_5=1; else INT6_5=0;
			if DP11k=5 | DP11ka=5 then INT6_6=1; else INT6_6=0;
			if DP13k=5 | DP13ka=5 then INT6_7=1; else INT6_7=0;
			if DP15ka=5 then INT6_8=1; else INT6_8=0;
		end;
		  else if DP27=5 then
				do;
					if DP27b1=5 | DP27b2=5 | DP27b3=5 then INT6_1=1; else INT6_1=0; 
					if DP27b5=5 then INT6_2=1; else INT6_2=0; 
	...
	**************;
	...
	else if (SU1=. or SU1=.K) and DP15kb ne 5 then INT7_1=.;
	else INT7_1=0;

	if SU1b=5 or DP15c=5 then INT7_2=1;
	else if (SU1=. or SU1=.K) and DP15kb ne 5 then INT7_2=.;
	else INT7_2=0;
...
run;
%mend;
%gen(cssaga4);%gen(cssaga4_f1);%gen(cssaga4_f1);%gen(cssaga4_f2);%gen(cssaga4_f3);
...
run;
%mend;
%freq(cssaga4);%freq(cssaga4_f1);%freq(cssaga4_f2);%freq(cssaga4_f3);


* macro to harvest achenbach data;

%macro harvest(achen,ssaga);
* Achenbach variables - only interviews performed during the same year as CSSAGA;
data tmp3; 
	set master.&achen;
		keep ind_id datefilled SC_AnxDep_raw SC_WithDep_raw SC_Somatic_raw SC_Aggressive_raw SC_RuleBreak_raw 
...
data tmp3; set tmp3; keep ind_id datefilled SC_AnxDep_raw SC_Somatic_raw SC_Aggressive_raw SC_RuleBreak_raw  
		     			  SC_Internal_raw SC_External_raw;

* Merging SSAGA, Achen and NEO;
data d.v21_int_&ssaga; merge d.v21_int_&ssaga tmp3; by ind_id;

data d.v21_int_&ssaga; set d.v21_int_&ssaga;
neo_n=.;neo_e=.;neo_o=.;neo_a=.;neo_c=.;SC_Intrusive_raw=.;SC_Withdrawn_raw=.;output;run;

%mend;
%harvest(ysr4,cssaga4);%harvest(ysr4_f1,cssaga4_f1);%harvest(ysr4_f2,cssaga4_f2);%harvest(ysr4_f3,cssaga4_f3);
...
%mend;
%daqscore(crv4,cssaga4);%daqscore(crv4_f1,cssaga4_f1);%daqscore(crv4_f2,cssaga4_f2);%daqscore(crv4_f3,cssaga4_f3);



proc contents data=d.v21_int_cssaga4;run;
